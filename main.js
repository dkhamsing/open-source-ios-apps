...
$(document).ready(function() {
  ...
  for(var i = 0; i < scripts.length; i++) {
    $.cachedScript(scripts[i].url, { scriptObj: scripts[i] }).always(function(data, textStatus) {
      loadedScripts++;
      var progress = Math.round(loadedScripts / scripts.length * 100);
      $("#splash-page .progress-bar").attr("aria-valuenow", progress).width(progress + "%").html("<span class=\"sr-only\">" + progress + "% Complete" + "</span>");
